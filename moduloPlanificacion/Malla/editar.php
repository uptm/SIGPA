...
<div class="row">
	<div class="col-xs-12">
		<h1 class="page-header">Editar malla</h1>
	</div>
</div>

<div class="row">
	<div class="col-xs-12">
		<form name="malla" method="POST" action="moduloPlanificacion/Malla/modificar.php" data-exe="embem('moduloPlanificacion/Malla/index.php', '#page-wrapper')" role="form">
			<div class="form-group">
				Código:
				<input type="text" name="id" placeholder="Código" value="<?= $malla->id; ?>" class="form-control" pattern="^[a-záéíóúñA-ZÁÉÍÓÚÑ0-9]+(\-[a-záéíóúñA-ZÁÉÍÓÚÑ0-9]+)*$" onKeyUp="if(this.value != document.malla.idAnt.value) Verif(this)" required="required" />
				<input type="hidden" name="idAnt" value="<?= $malla->id; ?>" />
				<p class="help-block">Solo están permitidos caracteres alfanuméricos y guiones. Ej: PNFI-2012.</p>
			</div>

			<div class="form-group">
				Fecha de creación:
				<input type="text" name="fecha" value="<?php $fecha = explode("-", $malla->fecha); echo "$fecha[2]/$fecha[1]/$fecha[0]"; ?>" placeholder="Fecha de creación" class="form-control datePicker" pattern="^((0?[1-9]|[12][0-9])/0?2|((0?[1-9]|[12][0-9])|30)/(0?4|0?6|0?9|11)|((0?[1-9]|[12][0-9])|3[01])/(0?1|0?3|0?5|0?7|0?8|10|12))/[0-9]+$" required="required" />
...
?>
					...
					<option value="<?= $carrera->id ?>" <?php if($malla->carrera == $carrera->id) echo "selected=\"selected\""; ?>><?= $carrera->nombre ?></option>";
...
?>
					...
					<option value="<?= $estructura->id ?>" <?php if($malla->estructuraID == $estructura->id) echo "selected=\"selected\""; ?>><?= $estructura->nombre ?></option>";
...
<?php
	$auxSedes = explode("&", $malla->sedes);

	foreach($auxSedes as $sede)
		$sedes[] = $sede;
...
?>
					...
					<tr id="<?= "$periodo->id-$subperiodo->id"; ?>" class="<?= $unidadCurricular->id; ?>"><td>
						<div class="form-group row">
							<div class="form-group col-xs-11">
								<strong><?= "$unidadCurricular->nombre ($unidadCurricular->id)"; ?></strong>
								<input type="hidden" name="unidadCurricular<?= $n; ?>" value="<?= $unidadCurricular->id; ?>" />
