...
// --------------------
	...
	$sql = "insert into eje values(default, '$nombre')";
	$exe = pg_query($sigpa, $sql);

// Si se guardo el eje correctamente

	if($exe) {

	// Agregar elemento al registro de acciones realizadas

		$sql = "insert into historial values('" . time() . "', '$_SESSION[nombre] $_SESSION[apellido] ($_SESSION[cedula])', 'Se agregó el eje <strong>$nombre</strong>', '" . htmlspecialchars($sql, ENT_QUOTES) . "')";
		$exe = pg_query($sigpa, $sql);
...
// Si ocurrio un error guardando el eje
