...
// --------------------
	...
	$sql = "update area set nombre='$nombre' where nombre='$nombreAnt'";
	$exe = pg_query($sigpa, $sql);
...
// Si se modificó el área
	...
	// Agregar elemento al registro de acciones realizadas
		...
		$sql = "insert into historial values('" . time() . "', '$_SESSION[nombre] $_SESSION[apellido] ($_SESSION[cedula])', 'Se modificó el área <strong>$nombreAnt</strong>', '" . htmlspecialchars($sql, ENT_QUOTES) . "')";
		$exe = pg_query($sigpa, $sql);
...
// Si ocurrió un error modificando el área
